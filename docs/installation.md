...
This guide will explain how to install the `hashgard` and `hashgardcli` entrypoints onto your system. With these installed on a server, you can participate in the mainnet as either a [Full Node](./join-mainnet.md) or a [Validator](./validators/validator-setup.md).
...
Next, let's install the latest version of Hashgard. Make sure you `git checkout` the correct [released version](https://gitlab.hashgard.com/hashgard/hashgard/releases).
...
That will install the `hashgard` and `hashgardcli` binaries. Verify that everything is OK:
