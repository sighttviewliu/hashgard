...
This document explains how the genesis file of the Cosmos Hub mainnet is structured. It also explains how you can build a genesis file for your own `hashgard` testnet.
...
The genesis file is stored in `~/.hashgard/config/genesis.toml`.
...
This command will add all the `gentxs` stored in `~/.hashgard/config/gentx` to the genesis file. In order to create a genesis transaction, click [here](./validators/validator-setup.md#participate-in-genesis-as-a-validator).
