# Hashgard 教程

欢迎来到 **Hashgard 区块链程序: `hashgard`** 教程.

## Hashgard 是什么?

- [Hashgard](./what-is-hashgard.md)

## 加入 Hashgard 测试网

- [安装 `hashgard`](./installation.md)
- [运行一个全节点并加入测试网](https://github.com/hashgard/testnets/blob/master/docs_CN/README.md)
- [升级为验证人节点](./validators/validator-setup.md)

## 部署你自己的 `hashgard` 私有测试网

- [部署自己的 `hashgard` 测试网](./deploy-testnet.md)

## 额外资源

- [CLI客户端文档](./hashgardcli.md)
- [验证人节点介绍](./validators/overview.md)
- [验证人节点 FAQ](./validators/validator-faq.md)
- [验证人节点安全考量](./validators/security.md)

