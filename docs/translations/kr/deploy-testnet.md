...
해당 문서는 `hashgard` 노드 네트워크를 구축하는 세가지 방법을 제시합니다. 각 모델은 다른 이용 사례에 특화되어 있습니다.
...
`localnet` 커맨드를 운영하기 위한 `hashgard` 바이너리(리눅스)와 `tendermint/hashgardnode` docker 이미지를 생성합니다. 해당 바이너리는 컨테이너에 마운팅 되며 업데이트를 통해 이미지를 리빌드 하실 수 있습니다.
...
이 커맨드는 4개 노드로 구성되어있는 네트워크를 hashgardnode 이미지를 기반으로 생성합니다. 각 노드의 포트는 하단 테이블에서 확인하실 수 있습니다:
...
`make localnet-start`는 `hashgard testnet` 명령을 호출하여 4개 노드로 구성된 테스트넷에 필요한 파일을 `./build`에 저장합니다. 이 명령은 `./build` 디렉토리에 다수의 파일을 내보냅니다.
...
각 `./build/nodeN` 디렉토리는 각자 컨테이너 안에 있는 `/hashgard`에 마운팅 됩니다.

### 로깅

로그는 각 `./build/nodeN/hashgard/gaia.log`에 저장됩니다. 로그는 docker를 통해서 바로 확인하실 수도 있습니다:
