...
토큰 스테이킹을 통해 `gardvalconspub`로 새로운 밸리데이터를 생성할 수 있습니다. 본인의 밸리데이터 pubkey를 확인하기 위해서는 다음 명령어를 입력하세요:
...
__참고__: `consensus_pubkey` 값이 지정되지 않은 경우, 기본적으로 `hashgard tendermint show-validator` 의 값으로 설정됩니다. `key_name`은 트랙잭션을 서명할때 이용되는 프라이빗키의 명칭입니다.
...
__참고__: 이 명령어는 제네시스에서의 처리를 위해 `gentx`를 `~/.hashgard/config/gentx`에 저장합니다.

::: tip
명령어 플래그에 대한 정보는 `hashgard gentx --help`를 사용에 확인하십시오.
:::
...
우선 `genesis.json`파일을 `hashgard`의 config 디렉토리로 가져옵니다.

Fetch the `genesis.json` file into `hashgard`'s config directory.
...
모든 제네시스 트랜잭션을 받으시고 `~/.hashgard/config/gentx`에 저장하셨다면 다음 명령어를 실행하십시오:
...
밸리데이터가 자동 언본딩 되었습니다. `gaia-8000`의 경우, 100개 블록 중 50개의 블록에 투표하지 않을 경우 언본딩 됩니다. 블록은 대략 ~2초 마다 생성되기 때문에 ~100초 이상 비활성화 상태를 유지하는 밸리데이터는 언본딩 될 수 있습니다. 가장 흔한 이유는 `hashgard` 프로세스가 멈춘 경우입니다.

보팅 파워를 다시 밸리데이터에게 되돌리기 위해서, 우선 `hashgard`가 실행되고 있는지 확인하십시오. 만약 실행되고 있지 않은 경우 다음 명령어를 실행하십시오:
...
::: danger 경고
`hashgard`가 싱크되지 않은 상태에서 `unjail` 명령을 실행하실 경우, 검증인이 아직 '구속' 상태라는 메시지를 받게 됩니다.
:::
...
### 문제 #2: `too many open files`때문에 `hashgard`가 강제 종료됩니다

리눅스가 각 프로세스당 열 수 있는는 파일 수는 최대 `1024`개입니다. `hashgard`는 1024개 이상의 열게될 수 있음으로 프로세스가 중단될 수 있습니다. 가장 간편한 해결책은 `ulimit -n 4096` (열 수 있는 최대 파일 수)명령어를 입력하고 프로세스를 `hashgard start`로 재시작하는 것입니다. 만약 `systemd` 또는 다른 프로세스 매니저로 `hashgard`를 실행하신다면 해당 레벨에서 몇가지 설정을 해야합니다. 문제 해결 샘플 `systemd` 파일은 다음과 같습니다:
