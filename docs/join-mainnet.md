...
You can edit this `moniker` later, in the `~/.hashgard/config/config.toml` file:
...
You can edit the `~/.hashgard/config/hashgard.toml` file in order to enable the anti spam mechanism and reject incoming transactions with less than the minimum gas prices:
...
Fetch the testnet's `genesis.json` file into `hashgard`'s config directory.

```bash
mkdir -p $HOME/.hashgard/config
curl https://raw.githubusercontent.com/cosmos/launch/master/genesis.json > $HOME/.hashgard/config/genesis.json
```
...
Your node needs to know how to find peers. You'll need to add healthy seed nodes to `$HOME/.hashgard/config/config.toml`. The [`launch`](https://github.com/cosmos/launch) repo contains links to some seed nodes.
...
Your full-node keeps unconfirmed transactions in its mempool. In order to protect it from spam, it is better to set a `minimum-gas-prices` that the transaction must meet in order to be accepted in your node's mempool. This parameter can be set in the following file `~/.hashgard/config/hashgard.toml`.
...
```bash
hashgard export > [filename].json
```

You can also export state from a particular height (at the end of processing the block of that height):

```bash
hashgard export --height [height] > [filename].json
```

If you plan to start a new network from the exported state, export with the `--for-zero-height` flag:

```bash
hashgard export --height [height] --for-zero-height > [filename].json
```
...
```bash
hashgard start --assert-invariants-blockly
```
